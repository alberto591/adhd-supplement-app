# ADR 0034: App Store Compliance (Privacy & Wellness Safety)

## Status
Accepted

## Context
Apple's App Store guidelines for wellness and health apps (Section 1.4.1 and 5.1) require explicit wellness disclaimers and detailed privacy manifest declarations for 2024+ submissions. To avoid rejection and ensure patient safety, the app needed a structured way to declare its data usage and a mandatory mechanism for users to acknowledge the "not wellness advice" nature of the tool.
...
2.  **Mandatory Onboarding Disclaimer**: Integrated a high-contrast `WellnessDisclaimerWidget` as the absolute first step of the `QuickSetupWizardScreen`.
...
5.  **Micro-copy Optimization**: Used Focus-friendly formatting (bullets, bolding) to ensure the disclaimer is readable and not just a "wall of text" that get ignored.
