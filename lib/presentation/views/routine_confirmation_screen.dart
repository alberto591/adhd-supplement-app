...
class RoutineOverrideConfirmationScreen extends StatefulWidget {
  final SupplementCompatibility compatibility;

  const RoutineOverrideConfirmationScreen({
    super.key,
    required this.compatibility,
  });

  @override
  State<RoutineOverrideConfirmationScreen> createState() =>
      _RoutineOverrideConfirmationScreenState();
}

class _RoutineOverrideConfirmationScreenState
    extends State<RoutineOverrideConfirmationScreen> {
  ...
  Future<void> _handleOverride() async {
    ...
    try {
      final safetyViewModel = context.read<RoutineSafetyViewModel>();
      // Log with a default reason since the UI text explicitly says "Confirmed with Advisor"
      await safetyViewModel.overrideCompatibility(
          widget.compatibility.id, "Confirmed with Advisor (Quick Action)");
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      widget.compatibility.description,
                                      style: TextStyle(
                                        fontSize: 14,
                                        height: 1.5,
                                        color: isDark
