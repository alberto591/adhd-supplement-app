# Privacy Policy for NeuroStack

**Last Updated:** January 25, 2026

## Introduction

NeuroStack ("we," "our," or "us") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application.

## Information We Collect

### Personal Information
- **Email Address**: Used for account creation and authentication
- **Display Name**: Optional profile information
- **User ID**: Automatically generated unique identifier

### Health & Wellness Data
- **Supplement Tracking**: Information about supplements you track (names, dosages, schedules)
- **Daily Logs**: Records of when you take supplements
- **Focus Type**: Optional self-reported diagnosis information
- **Symptom Check-ins**: Optional mood and focus ratings

### Usage Data
- **App Analytics**: How you use the app (screens visited, features used)
- **Crash Reports**: Technical information when the app crashes
- **Device Information**: Device type, OS version, app version

### Purchase Information
- **Subscription Status**: Whether you have an active subscription
- **Purchase History**: Managed through Apple App Store/Google Play Store

## How We Use Your Information

We use the collected information to:
- Provide and maintain the app's core functionality
- Send reminder notifications for your supplement schedule
- Personalize your experience
- Improve app performance and fix bugs
- Process in-app purchases
- Comply with legal obligations

## Data Storage

- **Cloud Storage**: Your data is stored securely using Firebase (Google Cloud Platform)
- **Local Storage**: Some data is cached locally on your device for offline access
- **Encryption**: Data is encrypted in transit using industry-standard SSL/TLS

## Data Sharing

We **DO NOT** sell your personal information. We may share data with:

### Service Providers
- **Firebase/Google Cloud**: Backend infrastructure and authentication
- **RevenueCat**: Subscription management
- **Apple/Google**: In-app purchase processing

### Legal Requirements
We may disclose your information if required by law or to:
- Comply with legal processes
- Protect our rights and safety
- Prevent fraud or abuse

## Your Rights

You have the right to:
- **Access**: Request a copy of your data
- **Delete**: Request deletion of your account and data
- **Modify**: Update your profile information
- **Export**: Download your supplement logs and history
- **Opt-out**: Disable analytics and crash reporting

### How to Exercise Your Rights
- **Delete Account**: Settings → Privacy & Security → Delete All My Data
- **Web Request**: [neurostack-app.web.app/delete-data](https://neurostack-app.web.app/delete-data)
- **Email**: Send a request to support@neurostack.app
- **Disable Analytics**: Settings → Privacy & Security → Share Analytics (toggle off)

## Data Retention

- **Active Accounts**: Data retained while your account is active
- **Deleted Accounts**: Data permanently deleted within 30 days
- **Legal Requirements**: Some data may be retained longer if required by law

## Children's Privacy

NeuroStack is not intended for children under 13. We do not knowingly collect information from children under 13. If you believe we have collected such information, please contact us immediately.

## Third-Party Services

Our app uses third-party services that may collect information:
- **Firebase Authentication & Firestore**: [Privacy Policy](https://firebase.google.com/support/privacy)
- **Firebase Analytics**: [Privacy Policy](https://firebase.google.com/support/privacy)
- **RevenueCat**: [Privacy Policy](https://www.revenuecat.com/privacy)

## Cookies and Tracking

We do not use cookies. We use Firebase Analytics to understand app usage patterns. You can disable this in Settings.

## Security

We implement appropriate security measures including:
- Encrypted data transmission (SSL/TLS)
- Secure authentication (Firebase Auth)
- Regular security updates
- Access controls and monitoring

However, no method of transmission over the internet is 100% secure.

## Changes to This Policy

We may update this Privacy Policy from time to time. We will notify you of significant changes by:
- Posting the new policy in the app
- Updating the "Last Updated" date
- Sending an in-app notification (for material changes)

## Clinical Disclaimer

NeuroStack is for informational and tracking purposes only. It is not a substitute for professional clinical advice, diagnosis, or treatment. Always consult your healthcare provider before starting any supplement regimen.

## Contact Us

If you have questions about this Privacy Policy or your data:

**Email**: support@neurostack.app  
**In-App**: Settings → Help Center → Contact Support

---

## Consent

By using NeuroStack, you consent to this Privacy Policy and agree to its terms.

---

**NeuroStack**  
Version 2.4.1  
© 2026 All Rights Reserved
